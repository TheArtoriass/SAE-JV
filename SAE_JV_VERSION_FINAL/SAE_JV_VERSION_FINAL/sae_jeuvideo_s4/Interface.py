...
def choisir_skin(choixtype):
    ...
    try:
        ...
        if choixtype == 1:  # Si le joueur est humain , on lui demande de choisir un skin
        ...
        if choixtype == 2: # Si l'IA est en jeu , on lui attribue un skin aléatoire
            
            # Obtenir le chemin du répertoire contenant les skins
            repertoire_skins = os.path.join(repertoire_script, 'img', 'personne')
            # Obtenir la liste des fichiers dans le répertoire des skins
            fichiers = [f for f in os.listdir(repertoire_skins) if os.path.isfile(os.path.join(repertoire_skins, f)) and f.startswith('Joueur') and f.endswith('.jpeg')]
            skin_choisi = random.choice(fichiers)
            return os.path.join(repertoire_script, 'img', 'personne', skin_choisi)
...
def choisir_classe(choixtype):
    ...
    try:
        ...
        if choixtype == 1: # Si le joueur est humain , on lui demande de choisir une classe de son choix
        ...
        if choixtype == 2: # Si le joueur est une IA , on lui attribue une classe aléatoire
            
            classe = random.randint(1, 4) # Choisir une classe entre 1 et 4
            if classe == 1:
                return Joueur1
            elif classe == 2:
                return Joueur2
...
def choisir_personnages():
    ...
    try:
        ...
        id_partie = connexion.inserer_donnees_partie(nombre_joueurs, 0)  # Insérer le nombre de joueurs dans la base de données
        ...
        for i in range(nombre_joueurs):
            ...
            choixtype = tkinter.simpledialog.askinteger("Type de joueur", f"Joueur {i+1}: 1 pour Joueur, 2 pour IA", minvalue=1, maxvalue=2)
            
            
            if choixtype == 1: # Pseudo pour les joueurs
                ...
                connexion.inserer_donnees_joueur(pseudo) # Insérer le pseudo du joueur dans la base de données
                
                classe = choisir_classe(choixtype)
                skin = choisir_skin(choixtype)
                
                
            elif choixtype == 2:  # Pseudo pour les IA
                pseudo = f"IA {i-1 +1}"
        ...
        return id_partie, joueurs, pseudos
    except Exception as e:
