...
from connexion import inserer_donnees_partie, inserer_donnees_kill
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    # Initialisation nombre d'events ramassés à 0
    nb_events_ramasses = 0


    id_partie, joueurs, pseudos = choisir_personnages()
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    # Création du graphe pour le chemin
    G = nx.Graph()
    for case in chemin:
        G.add_node(case)
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                if joueur_actif.pseudo.startswith("IA"): # Déplacement de l'IA
                    print("Déplacement de l'IA")
                    
                    # Si le joueur appuie sur la touche échap, on met le jeu en pause
                    if evenement.key == pygame.K_ESCAPE:
                        pause_active = True
                      
                    coor_prec = (joueur_actif.x, joueur_actif.y)
                    
                
                    # Choix de la direction vers l'avant
                    direction = None
                    

                    # Déplacement dans la direction choisie si possible
                    while de_restant > 0 and not tour_complet:
                        
                        if (joueur_actif.x + 1, joueur_actif.y) in chemin and joueur_actif.x < LARGEUR_ECRAN // taille_case: 
                            direction = 'droite'
                        elif (joueur_actif.x, joueur_actif.y + 1) in chemin and joueur_actif.y < HAUTEUR_ECRAN // taille_case:
                            direction = 'bas'    
                        elif (joueur_actif.x, joueur_actif.y - 1) in chemin and (joueur_actif.x, joueur_actif.y - 1) not in cases_occupees:
                            direction = 'haut'       
                        elif (joueur_actif.x - 1, joueur_actif.y) in chemin and joueur_actif.x > 0:
                            direction = 'gauche'        
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#                                                           Le combat si on croise un ennemi ( IA )
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#---------------------------------------------------------------------------------------------------------------------------------------
#                                                 Déplacements des joueurs humains
#---------------------------------------------------------------------------------------------------------------------------------------
                
                elif joueur_actif.pseudo != "IA":
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                    ...
                    if de_restant == 0:
                        ...
                        # Incrémentation du nombre d'évènements ramassés
                        nb_events_ramasses += 1
                        
                        # On insère le nombre d'évènements ramassés dans la base de données
                        id_partie = inserer_donnees_partie(len(joueurs), nb_events_ramasses, id_partie)
