...
import connexion
from Interface import choisir_personnages
...
class Combat:
    ...
    def lancer_combat(self, pseudos):
        ...
        try:
            ...
            if self.joueur_actif.pseudo.startswith("IA"):
            
                # L'IA choisit un type d'attaque aléatoire parmi les 3 types
                self.type_attaque = random.choice(["normal", "lente", "rapide"])
                self.boucle_combat(pseudos)
            ...
            elif self.joueur_actif.pseudo != "IA":
    ...
    def boucle_combat(self, pseudos):
        try:
            ...
            while self.joueur_actif.vie > 0 and self.ennemi.vieE > 0:
                ...
                if self.joueur_actif.vie <= 0:
                    ...
                    if self.joueur_index < len(pseudos):
                        
                        pseudos_joueur_actif = pseudos[self.joueur_index]
                        connexion.inserer_donnees_kill(self.ennemi.nomE, pseudos_joueur_actif)
                    ...
                    if self.joueur_index < len(pseudos):
                        pseudos_joueur_actif = pseudos[self.joueur_index]
                        connexion.inserer_donnees_kill(pseudos_joueur_actif, self.ennemi.nomE)
